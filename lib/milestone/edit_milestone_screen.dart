...
class _EditMilestoneScreenState extends State<EditMilestoneScreen> {
  ...
  void initState() {
    ...
    _reasonController = TextEditingController(text: widget.milestone.reason ?? '');
    _selectedYear = widget.milestone.targetYear;
    _selectedMonth = widget.milestone.targetMonth;
    _selectedColor = widget.milestone.color;
  ...
  Future<void> _handleUpdate() async {
    ...
    try {
      final response = await ApiService.updateMilestone(
      ...
      if (response['success'] == true) {
        ...
        _showError(response['error'] ?? 'Failed to update milestone');
      }
    } catch (e) {
  ...
  Future<void> _handleDelete() async {
    ...
    try {
      final response = await ApiService.deleteMilestone(widget.milestone.id);
      ...
      if (response['success'] == true) {
        ...
        _showError(response['error'] ?? 'Failed to delete milestone');
      }
    } catch (e) {
      if (mounted) _showError('Connection error');
    } finally {
                      ...
                      child: Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<int>(
                            ...
                            icon: const Icon(Icons.keyboard_arrow_down, color: Color(0xFFFF6B35)),
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: isSelected
                              ? [
                                  BoxShadow(
                                    color: Color(int.parse(color['hex'].replaceFirst('#', '0xFF'))).withAlpha((255 * 0.5).round()),
                                    blurRadius: 12,
                                    spreadRadius: 2,
                                  ),
                                ]
